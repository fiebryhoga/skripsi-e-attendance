<x-app-layout>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @foreach($todayAbsences as $absense)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3">
                                            <a href="{{ route('admin.homeroom.student.show', $absense->student->id) }}" class="font-bold text-gray-800 hover:text-indigo-600 hover:underline transition-colors flex items-center gap-1 group">
                                                {{ $absense->student->name }}
            ...
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                ...
                @else
                            ...
                            <li class="flex gap-3 items-start text-sm pb-4 border-b border-gray-50 last:border-0">
                                ...
                                <div class="flex-shrink-0">
                                    @if($violation->bukti_foto)
                                        <button onclick="openPhotoModal('{{ Storage::url($violation->bukti_foto) }}', '{{ $violation->student->name }}')" 
                                            ...
                                            <img src="{{ Storage::url($violation->bukti_foto) }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300" alt="Bukti">
                                ...
                                <div class="flex-1">
                                    <div class="flex justify-between">
                                        ...
                                        <a href="{{ route('admin.homeroom.student.show', $violation->student->id) }}" class="font-bold text-gray-800 hover:text-indigo-600 hover:underline">
                    ...
                    <div class="mt-4 text-center">
                        <a href="{{ route('admin.homeroom.violations', $classroom->id) }}" class="text-xs text-indigo-600 font-bold hover:underline">
                            Lihat Semua Pelanggaran Kelas Ini &rarr;
                        </a>
                    </div>
                @endif
            </div>
                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex flex-col">
                                            <a href="{{ route('admin.homeroom.student.show', $student->id) }}" class="font-medium text-gray-800 hover:text-indigo-600 hover:underline transition-colors group flex items-center gap-1">
                                                {{ $student->name }}
                                            ...
                                            <span class="text-[10px] text-gray-400">{{ $student->nis ?? '' }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <a href="{{ route('admin.homeroom.student.show', $student->id) }}" 
                                           class="inline-flex items-center px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-xs font-bold border border-indigo-100 hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-all shadow-sm">
                                            Lihat Detail
                                        </a>
                                    </td>
                                </tr>
    ...
    <script>
        function openPhotoModal(imageUrl, studentName) {
            ...
            document.getElementById('modalStudentName').innerText = 'Bukti Pelanggaran: ' + studentName;

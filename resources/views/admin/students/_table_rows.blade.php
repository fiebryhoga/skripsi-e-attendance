...
<tr class="hover:bg-indigo-50/30 transition-colors duration-200 group cursor-pointer border-b border-gray-50" onclick="window.location='{{ route('admin.students.show', $student) }}'">
    
    <td class="px-8 py-4">
        <div class="flex items-center gap-4">
            <div class="relative">
                <img class="w-10 h-10 rounded-full object-cover shadow-sm ring-2 ring-white" 
                    src="{{ $student->photo ? Storage::url($student->photo) : 'https://ui-avatars.com/api/?name='.$student->name.'&background=C0C0C0' }}"
                     alt="Foto">
            </div>
            <div>
                ...
                <p class="text-xs text-gray-400">{{ $student->phone_parent ?? '-' }}</p>
            </div>
        </div>
    </td>
    ...
    <td class="px-6 py-4">
        @if($student->classroom)
            <div class="flex items-center gap-2">
                <div class="w-2 h-2 rounded-full {{ str_contains($student->classroom->name, 'X-') ? 'bg-blue-500' : (str_contains($student->classroom->name, 'XI-') ? 'bg-indigo-500' : 'bg-purple-500') }}"></div>
...
<tr>
    <td colspan="5" class="px-6 py-4 border-t border-gray-100">
        {{ $students->links() }}
    </td>
</tr>
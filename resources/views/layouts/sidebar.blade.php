<aside class="fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transition-transform duration-300 ease-in-out transform lg:translate-x-0 lg:static lg:inset-0 flex flex-col"
    ...
    <div class="px-6 py-6 border-b border-gray-100">
        <div class="flex items-center gap-3">
            ...
            <div class="overflow-hidden">
                <p class="text-sm font-bold text-gray-800 truncate">{{ Str::limit(Auth::user()->name, 16) }}</p>
                <p class="text-xs text-indigo-600 font-medium bg-indigo-50 inline-block px-2 py-0.5 rounded-full mt-1">{{ Auth::user()->role->label() }}</p>
            </div>
        </div>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-4 space-y-1">
        ...
        <a href="#" class="text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 group flex items-center px-3 py-3 text-sm font-medium rounded-xl transition-all duration-200">
